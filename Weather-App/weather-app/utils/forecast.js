...
//setting params for the forecast function 
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url, json:true }, (error, { body}) => {  //url (shorthand syntax)// data is automatically parsed here and the lower two statements are if errors occur the first statement is if there is no internet or another issue that doesnt allow user to access body 
        if (error){
            ...
            // this will print if user inputs and invalied adress so the body/website has been accessed but user input was not valid
        } else if (body.error) {
...
//exporting the forecast function to allow it to be used in other JS files
